...
//グラフを作り、整列した節点の列をつくる
busmapjs.sort_nodes = function(
	a_walk_array//: i_walk_array[]
)//: string[] //節点idの配列
{
	
	//グラフ1：全ての節点を含むグラフ
	const c_graph_1 = {"nodes": {}};//as i_graph_1;
	
	//グラフ2：グラフ1から次数2の節点を除いたグラフ
	//多重辺はないので問題ない。self-loopは？
	const c_graph_2 = {"nodes": {}, "links": {}};//as i_graph_2;
	
	const c_walk_array_2 = [];//as i_walk_array_2[];
	
	
	//入力データからグラフ1をつくる
	for (let i1 = 0; i1 < a_walk_array.length; i1++) {
		for (let i2 = 0; i2 < a_walk_array[i1]["node_array"].length; i2++) {
			const c_node_id = a_walk_array[i1]["node_array"][i2];//as string; //節点id
			...
			if (i2 !== 0) { //最初以外
				const c_from_node_id = a_walk_array[i1]["node_array"][i2 - 1];//as string; //前の節点の節点id
			...
			if (i2 !== a_walk_array[i1]["node_array"].length - 1) { //最後以外
				const c_to_node_id = a_walk_array[i1]["node_array"][i2 + 1];//as string; //後の節点の節点id
	...
	for (let i1 = 0; i1 < a_walk_array.length; i1++) {
		let l_head = null;//as string; //最初の節点id
		let l_node_array_1 = "_";//as string; //節点idを_区切りで順に繋げた文字列
		let l_node_array_2 = "_";//as string; //節点idを_区切りで逆順に繋げた文字列
	...
	*/
	...
	for (let i1 = 0; i1 < c_walk_array_2.length; i1++) {
		const c_first = c_walk_array_2[i1]["link_array"][0];//as i_first_last;
		const c_last = c_walk_array_2[i1]["link_array"][c_walk_array_2[i1]["link_array"].length - 1];//as i_first_last;
	...
	let l_count = 1;// as number; //連続した正整数
	...
	*/
	...
	const c_acyclic = [];//as i_acyclic;
	//2 ** l_count通りについて、ループの有無を判定し、点数をつける
	for (let i1 = 1; i1 < 2 ** l_count; i1++) {
		if (i1 % 1000 === 0) {
			console.log(String(2 ** l_count) + "中" + String(i1) + "個完了");
		}
		const c_next_nodes = {};//as i_next_nodes; //ここに隣接する節点の節点idを入れていく。
		for (let i2 in c_graph_2["links"]) {
			const c_direction = -1 + 2 * (Math.floor(i1 / (2 ** c_graph_2["links"][i2]["code"])) % 2);//as number; //向き（-1か1）
		...
		function f_check_cyclic() {
			...
			for (let i2 in c_next_nodes) {
				if (c_next_nodes[i2]["cyclic"] !== false) {
					let l_next_node_count = Object.keys(c_next_nodes[i2]["next_nodes"]).length;//as number;
		...
		let l_exist = false;//as boolean; //巡回があればtrue
		for (let i2 in c_next_nodes) {
			if (c_next_nodes[i2]["cyclic"] !== false) {
				l_exist = true; //巡回あり
			}
		...
		if (l_exist === false) { //巡回なし
			//点数をつける
			let l_score = 0;//as number; //点数（大きいほどよい）
			for (let i2 = 0; i2 < c_walk_array_2.length; i2++) {
				for (let i3 = 0; i3 < c_walk_array_2[i2]["link_array"].length - 1; i3++) {
					const c_section_1 = c_walk_array_2[i2]["link_array"][i3]["link_id"];//as string;
					const c_section_2 = c_walk_array_2[i2]["link_array"][i3 + 1]["link_id"];//as string;
	...
	//最大になる向き付け
	let l_max_score = Number.MIN_SAFE_INTEGER;//as number; //最高点
	let l_argmax_score = null;//as number; //最高点の向き付け
	...
	*/
	...
	const c_link_array = [];//as i_link_array[]; //最終的な並べ方
	//トポロジカルソート
	for (let i1 in c_graph_2["links"]) {
		f_add(i1);
		function f_add(
			...
			if (c_graph_2["links"][a]["add"] !== true) {
				...
				let l_next_node = null; //as string;
	...
	*/
	...
	const c_node_array = [];//as i_node_array[];
	
	for (let i1 = 0; i1 < c_link_array.length; i1++) {
		const c_ids = c_link_array[i1]["id"].split("_");//as string;
		const c_direction = c_link_array[i1]["direction"];//as number;
		if (c_direction === 1) {
			for (let i2 = 1; i2 < c_ids.length - 1; i2++) {
				let l_head = false;//as boolean; //始点はtrue
				let l_tail = false;//as boolean; //終点はtrue
				let l_delete = false;//as boolean;
				if (i2 === 1) {
					l_head = true;
	...
	for (let i1 in c_graph_2["nodes"]) {
		let l_exist = false;//as boolean; //あったらtrue
	...
	const c_node_array_2 = [];//as string[]; //節点idのみの列
	for (let i1 = 0; i1 < c_node_array.length; i1++) {
